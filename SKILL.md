...
**AI 执行任何笔记创建指令前，必须完成以下自检清单：**

| 检查项 | 验证内容 | 失败处理 |
|--------|---------|---------|
| □ 模板读取 | 是否已读取对应模板文件？结构是否严格匹配模板章节？（逐节对比） | 停止执行，报告"未找到模板" |
| □ type 值校验 | 是否在允许列表？（note-overview/note-experience/daily/weekly/todo/output/tool/idea） | 停止执行，报告"非法 type 值" |
| □ 路径合规 | 文件路径是否符合落点规则？（📚主题笔记/<🎨主题>/） | 询问用户确认路径 |
| □ 文件存在性 | 文件是否已存在？ | 是→询问用户（覆盖/跳过/重命名），否→继续 |
| □ YAML 完整性 | frontmatter 是否包含：created, modified, type, title, source(如有), tags？ | 补充缺失字段 |
| □ 子分类 emoji | 子分类文件夹是否有 emoji 前缀？（新建时扫描已有风格保持一致） | 按语义自动添加 emoji |
...
**日志展示规则：**
- 执行开始前：展示「指令 + 模板 + type值 + 目标路径」
- 执行完成后：展示「✓ 完成：概括读取/写入的笔记」
- 非笔记类指令（如 `-check`）：展示执行结果摘要，不复述上面的路径信息
- 执行失败时：展示「✗ 失败：具体原因 + 当前状态」
