...
### 2.9 主题索引自动维护（借鉴 Karpathy Wiki）

在 `📚主题笔记/` 根目录维护统一的 `index.md`，按 tags 多层分组组织：
...
**维护规则**：
1. **统一入口**：所有主题笔记在根目录 `index.md` 中索引
2. **多层分组**：按 tags 自动分组，区分外部摘要 vs 经验沉淀
3. **自动追加**：新笔记收录时，按 tags 分组追加
4. **过期清理**：「最近收录」只保留最近7天
5. **状态标记**：`-check` 扫描后更新标记
6. **双向链接**：使用 Obsidian `[[笔记名]]` 语法

**优势**：
- 单一入口，全局视图
- 按 tags 灵活分组，不受文件夹层级限制
- 一眼看到某主题下的所有类型内容（摘要+经验）
...
> ⚠️ **注意**：`obsidian-ai-notes skill配置` 是一个**含空格的完整目录名**，
> 创建时必须用引号包裹（如 `mkdir -p "<vault>/obsidian-ai-notes skill配置"`），
> **不可拆分为两个目录**。

```text
<vault>/
├── 📅每日笔记/                    # -day
├── 📚主题笔记/                    # -note / -exp（按主题自动归类，子目录动态创建）
├── 📆每周笔记/                    # -week
├── 📋待跟踪任务/                  # -task
├── 📝创作输出/                    # -output（自动创建子分类）
├── 🛠️tools/                      # -tool
├── Clippings/                     # 收件箱（-classify 扫描）
└── "obsidian-ai-notes skill配置"/ # ⚠️ 含空格的完整目录名，勿拆分
    ├── SKILL.md                   # 核心执行规范
    ├── README.md                  # 用户说明
    ├── templates/                 # 笔记模板
    │   ├── daily-note每日笔记.md
...
4. **健康检查**：模仿 lint， `-check` 输出结构化问题报告 + 修复建议
