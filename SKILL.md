...
**小红书专项决策树（强制，直接执行不需查外部文件）：**

> 原则：**能拿到 ≥90% 完整数据就用，拿不到先降级完成、再推荐升级。流程最多两步。**

```
第一步：探测高质量方案（秒级，并行检测）
├── MCP 服务运行中？（ps aux | grep xiaohongshu-mcp）
│   └── ✅ → 直接用 MCP/小红书 Skill 脚本 → 完成（>95%完整）
├── 链接含 xsec_token 且用户只需文字？
│   └── ✅ → web_fetch 抓取 → 完成（~50%，仅文字）
└── 都不满足 → 进入第二步

第二步：降级完成 + 推荐一个最小可执行方案
├── 先用 web_fetch/defuddle 降级抓取（别让用户等）→ 生成笔记
└── 完成后附提示（仅当 MCP 不可用时）：
    「💡 当前仅获取到文字，缺少图片和互动数据。
     安装小红书 MCP 可获得完整体验，一次扫码长期可用。
     说"帮我安装小红书 MCP"即可，AI 全程引导（约 3 分钟）。」
```
